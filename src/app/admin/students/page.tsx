...
export default async function AdminStudentsPage(
  ...
  const queryFilter: any = { role: "student" };
  if (q) {
    queryFilter.$or = [
      { name: { $regex: q, $options: "i" } },
      { email: { $regex: q, $options: "i" } },
      { "studentProfile.rollNumber": { $regex: q, $options: "i" } },
    ];
  }
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col justify-between gap-4 rounded-lg border border-slate-200 bg-white p-5 shadow-sm md:flex-row md:items-end">
        ...
        <form className="flex w-full flex-col gap-3 sm:flex-row xl:w-auto" action="/admin/students">
